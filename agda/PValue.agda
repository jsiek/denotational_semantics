...
data Value : Set where
  ...
  ν : Value {- A function. Needed for call-by-value to distinguish from diverging. -}
...
initial-fin : (ρ : Env) → (NE-ρ : nonempty-env ρ) → fin-env (initial-fin-env ρ NE-ρ)
initial-fin ρ NE-ρ x
    with NE-ρ x
... | ⟨ v , v∈ρx ⟩ =
      ⟨ v ∷ [] , ⟨ equal (λ {w refl → mem-here}) (λ {w mem-here → refl}) , (λ ()) ⟩ ⟩
...
{- an environment that maps x to D and any other variable y to something in ρ y. -}
...
single-fin {v}{x}{ρ}{NE-ρ} y
    with x ≟ y
... | yes refl =
    ⟨ v ∷ [] , ⟨ equal (λ { v₁ refl → mem-here}) (λ { v₁ mem-here → refl}) , (λ ()) ⟩ ⟩
... | no neq
    with NE-ρ y
... | ⟨ w , w∈ρy ⟩ =
    ⟨ w ∷ [] , ⟨ equal (λ { v₁ refl → mem-here}) (λ { v₁ mem-here → refl}) , (λ ()) ⟩ ⟩
...
ISWIM-Λ-▪-id {N}{ρ}{NE-ρ}{X} NE-X =
    Λ-▪-id {λ D → ⟦ N ⟧ (D • ρ)} (⟦⟧-continuous{N}{ρ}{NE-ρ}) (⟦⟧-monotone-one{N}) NE-X
...
℘-▪-≃ {B}{P}{f}{k} = equal G H
  where
  G : ℘ (B ⇒ P) f ▪ ℘ (base B) k ≲ ℘ P (f k)
  G w ⟨ V , ⟨ ⟨ k′ , ⟨ refl , w∈fk′ ⟩ ⟩ , ⟨ k′∈pk , _ ⟩ ⟩ ⟩
      with k′∈pk (const k′) mem-here
  ... | pkk′ rewrite k′∈℘k⇒k′≡k pkk′ = w∈fk′
  H : ℘ P (f k) ≲ ℘ (B ⇒ P) f ▪ ℘ (base B) k
...
V⊆𝕍c⇒𝕍sV {[]} V⊆𝕍c = tt
V⊆𝕍c⇒𝕍sV {v ∷ V} V⊆𝕍c = ⟨ V⊆𝕍c v mem-here , V⊆𝕍c⇒𝕍sV (λ u z → V⊆𝕍c u (mem-there z)) ⟩
...
⟦⟧⇒⇓ {L · M}{γ}{WF-op (WF-cons (WF-ast wfL) (WF-cons (WF-ast wfM) WF-nil)) _}{ρ}{w}
    𝔾ργ w∈LMρ = G
    where
    Part2 : ∀{c₁ c₂}{L M}{γ}{V w}
       → γ ⊢ L ⇓ c₁  →  𝕍 (V ↦ w) c₁
...
⟦⟧⇒⇓ {ƛ N}{γ}{WF-op (WF-cons (WF-bind (WF-ast wfN)) WF-nil) _}{ρ}{v} 𝔾ργ v∈⟦M⟧ρ =
