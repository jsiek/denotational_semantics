...
module NewCompilerSingle where
  ...
  concretize : (M : NewClos2.AST) → NewClos3Single.AST
  concretize (` x) = ABT.` x
  ...
  delay : (M : NewClos3Single.AST) → NewClos4Single.AST
  del-map-args : ∀ {n} → NewClos3Single.Args (replicate n ■) → NewClos4Single.Args (replicate n ■)
  delay (` x) = ABT.` x
