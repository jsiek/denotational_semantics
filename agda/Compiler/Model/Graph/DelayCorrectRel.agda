...
open import Model.Graph.DomainISWIM as D4
  renaming (Value to Value')
open import Model.Graph.OperationAnnFun as Op3
open import Model.Graph.OperationISWIM as Op4
open import Model.Graph.Clos3 as L3
open import Model.Graph.Clos4 as L4
...
open import Model.Graph.Delay using (delay; del-map-args)
open import NewEnv
...
module Model.Graph.DelayCorrect where
...
data _~to_ : Value → Value' → Set where
  ...
  {- this definition may still be wrong... 
     it might depend on the statement of the theorem -}
data _~tos_ where
  [] : ∀ {ds'} → [] ~tos ds'
  _∷_ : ∀ {d ds ds'} 
        → (d~ : All (d ~to_) ds') 
...
delay-preserve : ∀ M ρ ρ' → (ρ~ : ∀ i d → ρ i d → ∀ d' → d ~to d' → ρ' i d')
               → ∀ d → d ∈ ⟦ M ⟧ ρ 
               → ∀ d' → d ~to d' → d' ∈ ⟦ delay M ⟧' ρ'
delay-preserve (` x) ρ ρ' ρ~ = ρ~ x
delay-preserve (clos-op n ⦅ ! clear (bind (bind (ast N))) ,, fvs ⦆) ρ ρ' ρ~ 
   d ⟨ [] , ⟨ FV , ⟨ [] ⊢ν , ⟨ ⟨ w∈N , neV ⟩ , ⟨ FV⊆𝒯fvs , ⟨ neFV , refl ⟩ ⟩ ⟩ ⟩ ⟩ ⟩
   d' d~ = {!   !} 
delay-preserve (clos-op n ⦅ ! clear (bind (bind (ast N))) ,, fvs ⦆) ρ ρ' ρ~ 
   (FV ⊢ V ↦ w) ⟨ [] , ⟨ FV , ⟨ [] ⊢ V ↦ w , ⟨ ⟨ ⟨ w∈N , neV ⟩ , neFV' ⟩ , ⟨ FV⊆𝒯fvs , ⟨ neFV , refl ⟩ ⟩ ⟩ ⟩ ⟩ ⟩
   ⦅ FV' ↦ (V' ↦ w') ∣ (to-↦-fst FV~ V~ w~)
    = ⟨ FV' , ⟨ ⟨ ⟨ IHM , {!   !} ⟩ , {!   !} ⟩ , ⟨ {!   !} , {!   !} ⟩ ⟩ ⟩
   where
   IHM : {!   !}
   IHM = delay-preserve N (mem V • mem FV • (λ _ x → x ≡ ω)) 
                         (mem V' • mem FV' • (λ _ x → x ≡ ω)) {!   !} w w∈N w' w~
delay-preserve (clos-op n ⦅ ! clear (bind (bind (ast N))) ,, fvs ⦆) ρ ρ' ρ~ 
   (FV ⊢ V ↦ w) ⟨ [] , ⟨ FV , ⟨ [] ⊢ V ↦ w , ⟨ ⟨ ⟨ w∈N , neV ⟩ , neFV' ⟩ , ⟨ FV⊆𝒯fvs , ⟨ neFV , refl ⟩ ⟩ ⟩ ⟩ ⟩ ⟩
   ∣ FV' ⦆ (to-↦-snd FV~)
    = {!   !}
delay-preserve (app ⦅ M ,, N ,, Nil ⦆) ρ ρ' ρ~ 
  d ⟨ FV , ⟨ neFV , ⟨ V , ⟨ FV⊢V↦d∈M , ⟨ V⊆N , neV ⟩ ⟩ ⟩ ⟩ ⟩ 
  d' d~ = {!   !}
  {-
...
... | clos∈M' | V'⊆N' = 
  ...
  , ⟨ V'⊆N' , to-ne V neV ⟩ ⟩ ⟩
   ...
   -}
delay-preserve (lit B k ⦅ Nil ⦆) ρ ρ' ρ~ (const {B'} k') d∈ d' to-const
   with base-eq? B B'
... | yes refl = d∈
... | no neq = d∈
delay-preserve (tuple n ⦅ args ⦆) ρ ρ' ρ~ d d∈ d' d~ 
  = {!   !}
  {- G n args ρ d d∈
  where
  ...
  -}
delay-preserve (get i ⦅ M ,, Nil ⦆) ρ ρ' ρ~ d ⟨ ds , ⟨ i≤ , ⟨ ds∈ , refl ⟩ ⟩ ⟩ 
  d' d~ = {!    !} 
{-  ⟨ tos ds , ⟨ subst (Data.Nat._<_ i) (sym (tos-length ds)) i≤ 
  , ⟨ delay-preserve M ρ ∥ ds ∥ ds∈ , tos-nth ds i ⟩ ⟩ ⟩
  -}
delay-preserve (inl-op ⦅ M ,, Nil ⦆) ρ ρ' ρ~ (left v) v∈ (left d') (to-left d~) = 
  delay-preserve M ρ ρ' ρ~ v v∈ d' d~
delay-preserve (inr-op ⦅ M ,, Nil ⦆) ρ ρ' ρ~ (right v) v∈ (right d') (to-right d~) = 
  delay-preserve M ρ ρ' ρ~ v v∈ d' d~
delay-preserve (case-op ⦅ L ,, (⟩ M ,, (⟩ N ,, Nil)) ⦆) ρ ρ' ρ~ 
  d (inj₁ ⟨ v , ⟨ V , ⟨ LV∈ , d∈ ⟩ ⟩ ⟩) 
  d' d~ = inj₁ ⟨ {!   !} , ⟨ {!   !} , ⟨ {!   !} , {!   !} ⟩ ⟩ ⟩ 
{-
     inj₁ ⟨ to v , ⟨ tos V , ⟨ G , G2 ⟩ ⟩ ⟩
...
delay-preserve (clos-op n ⦅ ! clear (bind (bind (ast N))) ,, fvs ⦆) ρ d  
  ⟨ [] , ⟨ FV , ⟨ [] ⊢ν , ⟨ ⟨ w∈N , neV ⟩ , ⟨ FV⊆𝒯fvs , ⟨ neFV , refl ⟩ ⟩ ⟩ ⟩ ⟩ ⟩ =
...
delay-preserve (clos-op n ⦅ ! clear (bind (bind (ast N))) ,, fvs ⦆) ρ d  
  ⟨ [] , ⟨ FV , ⟨ [] ⊢ V ↦ w , ⟨ ⟨ ⟨ w∈N , neV ⟩ , neFV' ⟩ , ⟨ FV⊆𝒯fvs , ⟨ neFV , refl ⟩ ⟩ ⟩ ⟩ ⟩ ⟩ = 
