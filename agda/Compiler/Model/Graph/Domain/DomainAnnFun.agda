...
module Model.Graph.DomainAnnFun where
...
data Value : Set where
  ...
  _⊢_↦'_ : (FV : List Value) → (V : List Value) → (w : Value) → Value
      ...
      {- we include "shadow" versions as duplicates for writing a relation between values -}
...
↦-inj : ∀ {FV FV' V V' w w'} → FV ⊢ V ↦ w ≡ FV' ⊢ V' ↦ w'
      → FV ≡ FV' × V ≡ V' × w ≡ w'
