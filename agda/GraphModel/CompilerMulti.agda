...
module GraphModel.CompilerMulti where
  ...
  concretize : (M : NewClos2.AST) → NewClos3Multi.AST
  concretize (` x) = ABT.` x
  ...
  delay : (M : NewClos3Multi.AST) → NewClos4Multi.AST
  del-map-args : ∀ {n} → NewClos3Multi.Args (replicate n ■) → NewClos4Multi.Args (replicate n ■)
  delay (` x) = ABT.` x
