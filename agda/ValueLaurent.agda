...
SomeFun-⊑ {.(_ ↦ _)} {w} fv (⊑-fun x v⊑w v⊑w₁) = x



⊑-dom : ∀{u v} → u ⊑ v → SomeFun u
         → Σ[ v′ ∈ Value ] v′ ⊆ v × dom v′ ⊑ dom u
⊑-dom {.⊥} {v} ⊑-⊥ ()
...
u⊆v⊔w-inv : ∀{u v w : Value}
       → u ⊆ v ⊔ w
         ----------------------------------
       → Σ[ u₁ ∈ Value ] Σ[ u₂ ∈ Value ]
            (∀{u′} → u′ ∈ u → u′ ∈ u₁ ⊎ u′ ∈ u₂)
          × u₁ ⊆ v × u₂ ⊆ w
u⊆v⊔w-inv {⊥} {v} {w} uvw = ⟨ v , ⟨ w , ⟨ uvw , ⟨ (λ {x} x₁ → x₁) , (λ {x} x₁ → x₁) ⟩ ⟩ ⟩ ⟩
u⊆v⊔w-inv {const x} {v} {w} uvw = ⟨ v , ⟨ w , ⟨ uvw , ⟨ (λ {x₁} x₂ → x₂) , (λ {x₁} x₂ → x₂) ⟩ ⟩ ⟩ ⟩
u⊆v⊔w-inv {u ↦ u₁} {v} {w} uvw = ⟨ v , ⟨ w , ⟨ uvw , ⟨ (λ {x} x₁ → x₁) , (λ {x} x₁ → x₁) ⟩ ⟩ ⟩ ⟩
u⊆v⊔w-inv {u ⊔ u₁} {v} {w} uvw = ⟨ v , ⟨ w , ⟨ uvw , ⟨ (λ {x} x₁ → x₁) , (λ {x} x₁ → x₁) ⟩ ⟩ ⟩ ⟩
...
⊆-cod {u ↦ u₁} {⊥} u⊆v afu {w} w∈codu
    with u⊆v refl
... | ()    
⊆-cod {u₁ ⊔ u₂} {v} u⊆v afu {w} (inj₁ x) = ⊆-cod (λ {u} z → u⊆v (inj₁ z)) (proj₁ afu) x
⊆-cod {u₁ ⊔ u₂} {v} u⊆v afu {w} (inj₂ y) = ⊆-cod (λ {u} z → u⊆v (inj₂ z)) (proj₂ afu) y
⊆-cod {⊥} {const k} u⊆v ()
...
⊑-trans {u} {v₁ ⊔ v₂} {w} (⊑-conj-R2 u⊑v₂) v₁⊔v₂⊑w =
   let v₂⊑w = ⊔⊑L v₁⊔v₂⊑w in
   ⊑-trans u⊑v₂ v₂⊑w
⊑-trans {u₁ ↦ u₂} {v} {w} (⊑-fun fv dv⊑u₁ u₂⊑cv)
   v⊑w =
   let dw⊑dv : dom w ⊑ dom v
       dw⊑dv = {!!} in
   let cv⊑cw : cod v ⊑ cod w
       cv⊑cw = {!!} in
   
   ⊑-fun (SomeFun-⊑ fv v⊑w) (⊑-trans dw⊑dv dv⊑u₁) (⊑-trans u₂⊑cv cv⊑cw)
