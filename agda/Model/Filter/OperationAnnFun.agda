...
⋆ : DOp (𝒫 Value) (■ ∷ ■ ∷ [])
⋆ ⟨ D₁ , ⟨ D₂ , _ ⟩ ⟩ w = 
   Σ[ FV ∈ Value ] Σ[ V ∈ Value ] (FV ⊢ V ↦ w) ∈ D₁ × V ∈ D₂ 


annot : Value → Value → Value
annot (v ⊢ν) FV = FV ⊢ν
annot (v ⊢ V ↦ w) FV = FV ⊢ V ↦ w
annot (v ⊔ v₁) FV = annot v FV ⊔ annot v₁ FV
annot d FV = d
...
𝓅 (B ⇒ P) f _ ω = True
𝓅 (B ⇒ P) f _ (ω ⊢ν) = True
𝓅 (B ⇒ P) f _ (ω ⊢ const {B'} k ↦ w) with base-eq? B B'
...
⋆-mono ⟨ D , ⟨ E , _ ⟩ ⟩ ⟨ D' , ⟨ E' , _ ⟩ ⟩ ⟨ lift D⊆ , ⟨ lift E⊆ , _ ⟩ ⟩ = lift G
  where
  G : ⋆ ⟨ D , ⟨ E , ptt ⟩ ⟩ ⊆ ⋆ ⟨ D' , ⟨ E' , ptt ⟩ ⟩
  G d ⟨ FV , ⟨ V , ⟨ wv∈D , V∈E ⟩ ⟩ ⟩ = ⟨ FV , ⟨ V , ⟨ D⊆ (FV ⊢ V ↦ d) wv∈D , E⊆ V V∈E ⟩ ⟩ ⟩
...
𝒜⋆-mono : monotone (■ ∷ ■ ∷ []) ■ 𝒜⋆
𝒜⋆-mono ⟨ D , ⟨ E , _ ⟩ ⟩ ⟨ D' , ⟨ E' , _ ⟩ ⟩ ⟨ D⊆ , ⟨ E⊆ , _ ⟩ ⟩ = lift G
  where 
  G : 𝒜⋆ ⟨ D , ⟨ E , ptt ⟩ ⟩  ⊆ 𝒜⋆ ⟨ D' , ⟨ E' , ptt ⟩ ⟩
  G d ⟨ x , ⟨ V , ⟨ w , ⟨ V↦w∈D , ⟨ V∈E , refl ⟩ ⟩ ⟩ ⟩ ⟩ = 
     ⟨ x , ⟨ V , ⟨ w , ⟨ lower D⊆ (x ⊢ V ↦ w) V↦w∈D , ⟨ lower E⊆ V V∈E , refl ⟩ ⟩ ⟩ ⟩ ⟩
