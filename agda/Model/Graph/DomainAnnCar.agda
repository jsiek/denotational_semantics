...
module Model.Graph.DomainAnnCar where
...
data Value : Set where
  ...
  ⦅_,_⦆ : (u : Value) → (V : List Value) → Value  
...
fst-inj : ∀ {u u' V V'} → ⦅ u , V ⦆ ≡ ⦅ u' , V' ⦆ → u ≡ u' × V ≡ V'
fst-inj refl = ⟨ refl , refl ⟩

fst-inj-uncurried : ∀ {u u' V V'} → ⦅ u , V ⦆ ≡ ⦅ u' , V' ⦆ → ⟨ u , V ⟩ ≡ ⟨ u' , V' ⟩
fst-inj-uncurried refl = refl
...
⦅ u , V ⦆ d≟ ν = no (λ ())
⦅ u , V ⦆ d≟ ω = no (λ ())
⦅ u , V ⦆ d≟ ⦅ u' , V' ⦆ = map′ (uncurry (cong₂ ⦅_,_⦆)) fst-inj ((u d≟ u') ×-dec (V ds≟ V'))
⦅ u , V ⦆ d≟ ∣ V' ⦆ = no (λ ())
⦅ u , V ⦆ d≟ ∥ ds ∥ = no (λ ())
⦅ u , V ⦆ d≟ left v = no (λ ())
⦅ u , V ⦆ d≟ right v = no (λ ())
