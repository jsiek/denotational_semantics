...
module Model.Graph.OperationAnnCar where
...
open import Model.Graph.DomainAnnCar
...
cdr ⟨ D , _ ⟩ fv = (Σ[ FV ∈ List Value ] ∣ FV ⦆ ∈ D × fv ∈ mem FV)
                 ⊎ (Σ[ u ∈ Value ] Σ[ FV ∈ List Value ] ⦅ u , FV ⦆ ∈ D × fv ∈ mem FV )
...
cdr-mono ⟨ D , _ ⟩ ⟨ D' , _ ⟩ ⟨ (lift D⊆) , _ ⟩ = lift G
  where
  G : cdr ⟨ D , _ ⟩ ⊆ cdr ⟨ D' , _ ⟩
  G v (inj₁ ⟨ V , ⟨ V∈ , v∈V ⟩ ⟩) = inj₁ ⟨ V , ⟨ D⊆ ∣ V ⦆ V∈ , v∈V ⟩ ⟩
  G v (inj₂ ⟨ u , ⟨ V , ⟨ V∈ , v∈V ⟩ ⟩ ⟩) = inj₂ ⟨ u , ⟨ V , ⟨ D⊆ ⦅ u , V ⦆ V∈ , v∈V ⟩ ⟩ ⟩
...
car-consis ⟨ D , _ ⟩ ⟨ D' , _ ⟩ ⟨ (lift D~) , _ ⟩ = lift G
  ...
  G u u' ⟨ FV , u∈ ⟩ ⟨ FV' , u'∈ ⟩ = D~ ⦅ u , FV ⦆ ⦅ u' , FV' ⦆ u∈ u'∈

cdr-consis : consistent _~_ (■ ∷ []) ■ cdr
cdr-consis ⟨ D , _ ⟩ ⟨ D' , _ ⟩ ⟨ (lift D~) , _ ⟩ = lift G
  where
  G : Every _~_ (cdr ⟨ D , ptt ⟩) (cdr ⟨ D' , ptt ⟩)
  G v v' (inj₁ ⟨ V , ⟨ V∈ , v∈V ⟩ ⟩) (inj₁ ⟨ V' , ⟨ V'∈ , v'∈V' ⟩ ⟩)
    = ≈⇒Every V V' (D~ ∣ V ⦆ ∣ V' ⦆ V∈ V'∈) v v' v∈V v'∈V'
  G v v' (inj₁ ⟨ V , ⟨ V∈ , v∈V ⟩ ⟩) (inj₂ ⟨ u , ⟨ V' , ⟨ V'∈ , v'∈V' ⟩ ⟩ ⟩)
    = {!   !}
  G v v' (inj₂ ⟨ u , ⟨ V , ⟨ V∈ , v∈V ⟩ ⟩ ⟩) (inj₁ ⟨ V' , ⟨ V'∈ , v'∈V' ⟩ ⟩)
    = {!   !}
  G v v' (inj₂ ⟨ u , ⟨ V , ⟨ V∈ , v∈V ⟩ ⟩ ⟩) (inj₂ ⟨ u' , ⟨ V' , ⟨ V'∈ , v'∈V' ⟩ ⟩ ⟩)
    = {!   !}
