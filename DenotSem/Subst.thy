...
fun shift :: "int \<Rightarrow> nat \<Rightarrow> exp \<Rightarrow> exp" ("\<up>") where
  "shift k c (ENat n) = ENat n" |
  "shift k c (EPrim f e1 e2) = EPrim f (shift k c e1) (shift k c e2)" |
  "shift k c (EVar n) = (if c \<le> n then EVar (nat (int n + k)) else EVar n)" |
...
fun subst :: "nat \<Rightarrow> exp \<Rightarrow> exp \<Rightarrow> exp" ("[_\<mapsto>_]_" [79,79] 78) where
  ...
  "subst k v (EVar n) = (if k = n then v else EVar n)" |
  "subst k v (ELam e) = ELam  (subst (Suc k) (shift 1 0 v) e)" |
