...
\begin{align*}
  ...
  \ESEM{\lambda e }\rho &= \{ A \mid \WF{A} \text{ and } \FSEM{A}(e,\rho) \} \\
  \ESEM{e_1\app e_2}\rho &= \left\{ C\, \middle| \exists A,B.\;
      \begin{array}{l}
      A \in \ESEM{e_1}\rho,
      B \in \ESEM{e_2}\rho,\\
  ...
  \ESEM{f(e_1,e_2)}\rho &=
      \left\{ C\, \middle| \begin{array}{l}
       \exists A,B,n_1,n_2.\; A \in \ESEM{e_1}\rho, B \in \ESEM{e_2}\rho,\\
      \qquad A <: n_1, B <: n_2, \SEM{f}(n_1,n_2) <: C, \WF{C} 
      \end{array} \right\} \\
  \ESEM{\IF e_1 \THEN e_2 \ELSE e_3}\rho &=
    \left\{ B\, \middle| \exists A.\;
    \begin{array}{l}A \in \ESEM{e_1}\rho, A <: n,\\
           n = 0 \Rightarrow B \in \ESEM{e_3}\rho,\\
           n \neq 0 \Rightarrow B \in \ESEM{e_2}\rho
    \end{array}
    \right\}
    \\[2ex]
    \intertext{where}
   \FSEM{n}e\rho &= \mathit{false} \\
   \FSEM{A \sqcap B}e \rho &= \FSEM{A}e\rho \text{ and } \FSEM{B}e\rho\\
   \FSEM{A \to B}e \rho &= B \in \ESEM{e} (A, \rho)
\end{align*}
...
Now we can move on to lemmas about the denotational semantics that
will be needed to prove the Join Theorem. \\

\noindent \textbf{Lemma} (Weakening)
\begin{enumerate}
\item If $\FSEM{A}e \rho_1$, $\rho_1 <: \rho_2$ and
  $(\forall B \rho_1 \rho_2,\; B \in \ESEM{e}\rho_1, \rho_2 <: \rho_1
  \Rightarrow B \in \ESEM{e}\rho_2)$, then
  $\FSEM{A}e \rho_2$.
\item If $A \in \ESEM{e}\rho_1$ and $\rho_2 <: \rho_1$,
  then $A \in \ESEM{e}\rho_2$.
\end{enumerate}
(Part 1 is proved by induction on $A$. Part 2 is proved by induction
on $e$ and uses part 1.) \\

\noindent \textbf{Lemma} (Consistent Application) \\
%
