theory Denot
  imports LaurentValues Lambda Consistency
begin
...
fun E :: "exp \<Rightarrow> val list \<Rightarrow> val set" ("\<lbrakk>_\<rbrakk>_" [999] 1000) and
  ...
  Evar: "\<lbrakk>EVar k\<rbrakk> \<rho> = (if k < length \<rho> then {v. v \<sqsubseteq> \<rho>!k \<and> is_val v } else {})" |
  Elam: "\<lbrakk>ELam e\<rbrakk> \<rho> = { t. is_val t \<and> F t e \<rho> }" |
  Eapp: "\<lbrakk>EApp e1 e2\<rbrakk> \<rho> = { v'. \<exists> f v. f \<in> \<lbrakk>e1\<rbrakk>\<rho> \<and> v \<in> \<lbrakk>e2\<rbrakk>\<rho> \<and> v\<mapsto>v' \<sqsubseteq> f \<and> is_val f \<and> is_val v }" | 
