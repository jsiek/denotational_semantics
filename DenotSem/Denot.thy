...
fun E :: "exp \<Rightarrow> val list \<Rightarrow> val set" ("\<lbrakk>_\<rbrakk>_" [999] 1000) where
  ...
  Elam: "\<lbrakk>ELam e\<rbrakk> \<rho> = { VFun t|t. is_val (VFun t) 
                       \<and> (\<forall> v v'. v\<mapsto>v' \<in> set t \<longrightarrow> v' \<in> \<lbrakk>e\<rbrakk>(v#\<rho>)) }" |
  Eapp: "\<lbrakk>EApp e1 e2\<rbrakk> \<rho> = { v3. \<exists> f v2. VFun f \<in> \<lbrakk>e1\<rbrakk>\<rho> \<and> v2 \<in> \<lbrakk>e2\<rbrakk>\<rho> \<and> v2\<mapsto>v3 in f }" | 
