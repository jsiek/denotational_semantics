...
\setbeamersize{text margin left=3mm,text margin right=15mm}
...
%  \inference{}{\Gamma \vdash c : \Delta(c)}\\[1ex]
  ...
  \inference{\Gamma \vdash \lambda x.N : A & \Gamma \vdash \lambda x.N : B}
            {\Gamma \vdash \lambda x.N : A \AND B} \\[1ex]
  \inference{}
...
%   \SEM{c}\rho &= \Delta\SEM{c} \\
   ...
   \SEM{\lambda x. N}\rho &= \{ v_3 \mid \forall v_1 {\mapsto} v_2 \in \mathsf{graph}(v_3), \\
   & \qquad\quad\;\; v_2 \in \SEM{N}\rho(x{:=}v_1)\} 
 \end{align*}
\begin{align*}    
   \mathsf{graph}(\emptyset) &= \emptyset \\
   \mathsf{graph}(v \mapsto w) &= \{ v \mapsto w \} \\
   \mathsf{graph}(u \sqcup v) &= \mathsf{graph}(u) \cup \mathsf{graph}(v) 
\end{align*}
\end{minipage}
}
\end{tabular}
...
\hfill\fbox{$A <: B$}
\begin{gather*}
\inference{}{A <: A} \qquad \inference{A <: B \quad B <: C}{A <: C} \quad \inference{}{A <: \top}\\[2ex]
...
property into a rule in a sequent-style subtyping system (ITRS 2018).
\[
...
\[
\inference{\mathsf{graph}(B) \subseteq \mathsf{graph}(A) \\
           C <: \mathsf{dom}(B) & \mathsf{cod}(B) <: D}
          {A <: C \to D}
\]
where
\begin{align*}
