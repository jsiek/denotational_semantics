...
\frame{
  ...
  \item Compilation of Functional Languages
...
%  \inference{}{\Gamma \vdash c : \Delta(c)}\\[1ex]
  ...
  \inference{\Gamma \vdash \lambda x.N : A & \Gamma \vdash \lambda x.N : B}
            {\Gamma \vdash \lambda x.N : A \& B} \\[1ex]
  \inference{}
            {\Gamma \vdash \lambda x.N : \bot}
...
\end{gather*}
 \end{minipage}}
  ...
  \fbox{
   ...
   \footnotesize
    \begin{align*}
      v & ::= \emptyset \mid v \sqcup v \mid v \mapsto v  \\[2ex]
...
%   \SEM{c}\rho &= \Delta\SEM{c} \\
   ...
   \SEM{\lambda x. N}\rho &= \{ t \mid \forall v_1 \mapsto v_2 \in t, \\
     & \qquad\quad\;\; v_2 \in \SEM{N}\rho(x{:=}v_1)\} 
 \end{align*}
  \end{minipage}
  }
\end{tabular}
...
\ \\
but \textbf{not Full Abstraction}: \\
  $\qquad M \overset{\mathrm{ctx}}{=} N$ does not imply $\SEM{M} = \SEM{N}$ \\
\ \\
(but that's OK, we don't need that direction.)

}
%===============================================================================
\frame{
  \frametitle{Compilation of Functional Languages}
...
&f \app w \\
& \Downarrow \\
& \mathbf{fun}~ g(v, z) = v[0] + v[1] + z \\
&\LET w = 5, x = 3, y = 3 \IN \\
&\LET f = \langle g, \langle x , y \rangle \rangle \IN \\
&f[0] \app f[1]\app w
\end{align*}
...
%===============================================================================
\frame{
\frametitle{Challenge: the denotations of $\lambda$'s are relations}

Recall the example of common subexpression elimination:
\begin{align*}
&  \lam{f} (f \app 5) + (f \app 5) \\
&   \Downarrow \\
&  \lam{f} \LET x = (f \app 5) \IN x + x
...
\begin{gather*}
  ...
  \inference{v \sim v' \text{ implies } w \sim w'}
...
\begin{gather*}
  ...
  \inference{u \sim v & \WF{u} & \WF{v} }{\WF{u \sqcup v}}
\end{gather*}

Update the definition of $\SEM{-}$:
\begin{align*}
   \SEM{L~M}\rho &= \{ w \mid  \WF{v}, v\mapsto w \in \SEM{L}\rho, v\in \SEM{M}\rho \} \\
\end{align*}
...
\textbf{Lemma} (Consistency and Ordering)
\begin{itemize}
\item  If $u_1 \sim u_2$, $v_1 \sqsubseteq u_1$, and $v_2 \sqsubseteq u_2$,
  then $v_1 \sim v_2$.
\item If $u_1 \not\sim u_2$, $u_1 \sqsubseteq v_1$, $u_2 \sqsubseteq v_2$,
  then $v_1 \not\sim v_2$.
\end{itemize}

\ \\
but we get stuck on the following case for transitivity of $\sqsubseteq$ \\
because $v$ may not be well formed even though $u$ and $w$ are.
\[
\inference{u \sqsubseteq v & v \sqsubseteq w}
          {u \sqsubseteq w}
\]
...
The value information ordering $u \sqsubseteq v$ is just the
inverse of BCD Subtyping:

\hfill\fbox{$A <: B$}
\begin{gather*}
\inference{}{A <: A} \qquad \inference{A <: B \quad B <: C}{A <: C} \\[2ex]
\inference{}{A \& B <: A} \quad
\inference{}{A \& B <: B} \quad
\inference{C <: A \quad C <: B}{C <: A \& B} \\[2ex]
\inference{C <: A \quad B <: D}{A \to B <: C \to D}
\quad
\inference{}{(A\to B) \& (A \to C) <: A \to (B \& C)}
\end{gather*}

}
%===============================================================================
\frame{
\frametitle{Solution: Subtyping Alternative}
