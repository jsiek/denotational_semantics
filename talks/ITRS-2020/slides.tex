...
\frame{
  ...
  \item Challenges and Solutions
    ...
    \item Mechanizing Variables and Substitusion
    \item Language-generic meta-theory
    \item Intrinsic difficulties
    \end{itemize}
  \end{itemize}
...
&f \app w \\
& \Downarrow \\
& \mathbf{fun}~ g\,\rd{v \, z} = \rd{\mathsf{fst}\,v} + \rd{\mathsf{snd}\,v} + z \\
&\LET w = 5, x = 3, y = 3 \IN \\
&\LET f = \rd{\langle g, \langle x , y \rangle \rangle} \IN \\
...
\ \\
We use McBride's approach to de Bruijn indices. \\

\begin{lstlisting}
$(a \bullet f) \app 0 = a$
$(a \bullet f) \app (suc\app n) = f \app n$
  
$\Uparrow \rho$ = $\lambda x.\, suc \app (\rho \app x)$
  
rename $\rho$ x = $\rho$ x
rename $\rho$ ($\lambda$ N) = $\lambda$ (rename (0 $\bullet$ $\Uparrow \rho$) N)
rename $\rho$ (L $\cdot$ M) = (rename $\rho$ L) $\cdot$ (rename $\rho$ M)

$\Uparrow \sigma$ = $\lambda x.\,$rename suc $(\sigma \app x)$
